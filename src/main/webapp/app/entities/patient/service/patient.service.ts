...
type RestOf<T extends IPatient | NewPatient> = Omit<T, 'birthdate' | 'createdDate' | 'updatedDate'> & {
...
export class PatientService {
  ...
  protected convertDateFromClient<T extends IPatient | NewPatient | PartialUpdatePatient>(patient: T): RestOf<T> {
    return {
      ...
      createdDate: patient.createdDate?.toJSON() ?? null,
      updatedDate: patient.updatedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPatient: RestPatient): IPatient {
    return {
      ...
      updatedDate: restPatient.updatedDate ? dayjs(restPatient.updatedDate) : undefined,
    };
  }
